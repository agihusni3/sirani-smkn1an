...
<head>
  ...
  <style>
    /* Scoped styling for PTK portal inside SIRANI layout */
    .ptk-container {
      padding: 0;
      width: 100%;
    }
    ...
    .nav-pills .nav-link:hover:not(.active) {
      background-color: var(--bg-2, #F1F5F9);
...
<div class="app-container">
  ...
  <main class="main-content">
    ...
    {{-- NAV TABS INTERFACE --}}
    <ul class="nav nav-pills gap-2 mb-4 p-1.5 rounded-3 shadow-sm" id="ptkTab" role="tablist" style="background:var(--surface, #FFFFFF); border:1px solid var(--border, #E2E8F0); width:fit-content;">
      <li class="nav-item" role="presentation">
        <button class="nav-link active px-3 py-2 fw-bold d-flex align-items-center gap-2" id="tab-biodata-btn" data-bs-toggle="pill" data-bs-target="#tab-biodata" type="button" role="tab" style="font-size:13px;">
          <i class="bi bi-person-vcard"></i> Biodata Kepegawaian
        </button>
      </li>
      <li class="nav-item" role="presentation">
        <button class="nav-link px-3 py-2 fw-bold d-flex align-items-center gap-2" id="tab-berkas-btn" data-bs-toggle="pill" data-bs-target="#tab-berkas" type="button" role="tab" style="font-size:13px;">
      ...
      <li class="nav-item" role="presentation">
        <button class="nav-link px-3 py-2 fw-bold d-flex align-items-center gap-2" id="tab-presensi-btn" data-bs-toggle="pill" data-bs-target="#tab-presensi" type="button" role="tab" style="font-size:13px;">
    ...
    <div class="tab-content" id="ptkTabContent">
      ...
      <div class="tab-pane fade show active" id="tab-biodata" role="tabpanel" tabindex="0">
        <div class="row g-4">
          ...
          <div class="col-lg-6">
            <div class="card border-0 shadow-sm rounded-4 h-100" style="background:var(--surface, #FFFFFF); border:1px solid var(--border, #E2E8F0)!important;">
              <div class="card-header bg-transparent py-3 px-4 d-flex align-items-center gap-2" style="border-bottom:1px solid var(--border, #E2E8F0);">
                <i class="bi bi-person-lines-fill text-primary fs-5"></i>
                <h5 class="card-title fw-bold mb-0" style="font-size:15px; color:var(--text, #0F172A);">Identitas Diri &amp; Kependudukan</h5>
              </div>
                ...
                <table class="table table-borderless align-middle mb-0" style="font-size:13px;">
                  <tbody>
                    <tr>
                      ...
                      <td>: <strong style="color:var(--text, #0F172A);">{{ $guru->nama_lengkap ?: $guru->nama }}</strong></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                      ...
                      <td>: <span class="font-monospace" style="color:var(--text, #0F172A);">{{ $guru->nik ?: '—' }}</span></td>
                    </tr>
                    <tr>
          ...
          <div class="col-lg-6">
            <div class="card border-0 shadow-sm rounded-4 h-100" style="background:var(--surface, #FFFFFF); border:1px solid var(--border, #E2E8F0)!important;">
              <div class="card-header bg-transparent py-3 px-4 d-flex align-items-center gap-2" style="border-bottom:1px solid var(--border, #E2E8F0);">
                ...
                <h5 class="card-title fw-bold mb-0" style="font-size:15px; color:var(--text, #0F172A);">Status Kepegawaian &amp; Riwayat Jabatan</h5>
              </div>
              <div class="card-body p-4">
                <table class="table table-borderless align-middle mb-0" style="font-size:13px;">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td>: <strong style="color:var(--text, #0F172A);">{{ $guru->pangkat ?: '—' }} ({{ $guru->golongan_ruang ?: '—' }})</strong></td>
                    </tr>
                    <tr>
      ...
      <div class="tab-pane fade" id="tab-berkas" role="tabpanel" tabindex="0">
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden" style="background:var(--surface, #FFFFFF); border:1px solid var(--border, #E2E8F0)!important;">
          
          <div class="card-header bg-transparent py-3 px-4 d-flex flex-column flex-sm-row justify-content-between align-items-start align-items-sm-center gap-3" style="border-bottom:1px solid var(--border, #E2E8F0);">
            <div>
              <h5 class="fw-bold mb-0" style="font-size:15px; color:var(--text, #0F172A);">
          ...
          <div class="card-body p-0">
            @if($arsips->isEmpty())
              <div class="text-center py-5 px-3">
                <div class="d-inline-flex align-items-center justify-content-center rounded-circle mb-3" style="width:64px; height:64px; background:var(--bg-2, #F1F5F9); color:var(--text-muted, #94A3B8);">
                  <i class="bi bi-folder2-open fs-2"></i>
                </div>
                <h6 class="fw-bold mb-1" style="color:var(--text, #0F172A);">Belum Ada Berkas Digital Tersimpan</h6>
                  ...
                  <tbody>
                    @foreach($arsips as $idx => $dok)
                      ...
                      <tr>
                        ...
                        <td class="py-3 px-3">
                          <div class="fw-bold" style="color:var(--text, #0F172A);">{{ $dok->nama_dokumen }}</div>
                          <div class="text-muted" style="font-size:11px;">
                            Diunggah: {{ $dok->created_at ? $dok->created_at->translatedFormat('d M Y, H:i') : '—' }}
                          </div>
                        </td>
                        <td class="py-3 px-3 font-monospace small" style="color:var(--text, #0F172A);">
                          {{ $dok->nomor_dokumen ?: '—' }}
                        </td>
      ...
      <div class="tab-pane fade" id="tab-presensi" role="tabpanel" tabindex="0">
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden" style="background:var(--surface, #FFFFFF); border:1px solid var(--border, #E2E8F0)!important;">
          <div class="card-header bg-transparent py-3 px-4" style="border-bottom:1px solid var(--border, #E2E8F0);">
            <h5 class="fw-bold mb-0" style="font-size:15px; color:var(--text, #0F172A);">
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow-lg rounded-4">
      <div class="modal-header border-0 pb-0">
        <h5 class="modal-title fw-bold" id="modalUnggahBerkasLabel" style="color:var(--text, #0F172A);">
