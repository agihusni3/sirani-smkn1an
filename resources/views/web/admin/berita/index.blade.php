    ...
    <div class="panel" style="background:var(--bg-2); border:1px solid var(--border); padding:12px 18px; margin-bottom:16px; border-radius:12px;">
      <form method="GET" action="{{ route('admin.berita.index') }}" style="display:flex; flex-wrap:wrap; gap:10px; align-items:center; justify-content:space-between;">
        <div style="display:flex; flex-wrap:wrap; gap:8px; align-items:center; flex:1;">
          <input type="text" name="cari" value="{{ request('cari') }}" placeholder="Cari judul artikel atau kata kunci..." style="min-width:280px; flex:1; height:38px; padding:0 14px; font-size:12.5px; border-radius:8px; border:1px solid var(--border-2); background:var(--bg-3); color:var(--text); font-weight:600;">
          
          <select name="kategori" onchange="this.form.submit()" style="height:38px; padding:0 12px; font-size:12px; border-radius:8px; border:1px solid var(--border-2); background:var(--bg-3); color:var(--text); font-weight:700;">
          ...
          @if(request()->hasAny(['cari', 'kategori']))
            <a href="{{ route('admin.berita.index') }}" class="btn btn-sm" style="height:38px; padding:0 12px; display:inline-flex; align-items:center; justify-content:center; background:var(--bg-3); border:1px solid var(--border-2); color:var(--text); font-weight:700; font-size:12px; border-radius:8px; text-decoration:none;">
              Reset Filter
            </a>
          @endif
        </div>
      ...
      <div style="overflow-x:auto;">
        <table class="table" style="width:100%; border-collapse:collapse; font-size:12.5px; margin:0;">
          ...
          <tbody>
            @forelse($beritas as $b)
              <tr style="border-bottom:1px solid var(--border); vertical-align:middle;">
                <td style="padding:10px 14px;">
                  ...
                  @else
                    <div style="width:60px; height:42px; background:var(--bg-3); border-radius:6px; display:flex; align-items:center; justify-content:center; color:var(--text-3); font-size:16px; border:1px solid var(--border);">
                ...
                <td style="padding:10px 14px; font-size:11.5px; color:var(--text-2);">
                  <div style="font-weight:700; color:var(--text);">{{ $b->penulis->name ?? 'Admin Humas' }}</div>
            ...
            @empty
              <tr>
                <td colspan="7" style="text-align:center; padding:40px 16px; color:var(--text-3); font-weight:700;">
                  <i class="bi bi-journal-x" style="font-size:32px; display:block; margin-bottom:8px; opacity:0.6; color:var(--navy);"></i>
