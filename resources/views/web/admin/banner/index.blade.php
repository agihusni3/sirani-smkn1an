...
<div class="app-container">
  ...
  <main class="main-content">
    ...
    <div class="panel no-print" style="background:var(--bg-2); border:1px solid var(--border); padding:12px 18px; margin-bottom:14px; border-radius:var(--r-md); box-shadow:0 2px 8px rgba(0,0,0,0.02);">
      <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
        ...
        <div style="display:flex; align-items:center; gap:8px;">
          <a href="{{ route('admin.banner.create') }}" class="btn btn-sm" style="background:var(--navy); color:#ffffff; font-weight:800; border-radius:6px; font-size:12px; padding:7px 15px; border:none; display:inline-flex; align-items:center; gap:6px; box-shadow:0 2px 6px rgba(37,99,235,0.25); text-decoration:none;">
          ...
          <a href="{{ route('web.beranda') }}" target="_blank" class="btn btn-sm" style="background:var(--bg-3); border:1px solid var(--border-2); color:var(--text); font-weight:800; border-radius:6px; font-size:12px; padding:7px 14px; text-decoration:none;">
            <i class="bi bi-box-arrow-up-right" style="color:var(--navy);"></i> Lihat di Web
          </a>
        </div>
      </div>
    </div>
    ...
    @if(session('success'))
      <div class="panel" style="background:rgba(16,185,129,0.12); border:1px solid rgba(16,185,129,0.3); color:#10b981; padding:12px 16px; margin-bottom:14px; border-radius:var(--r-sm); font-size:13px; font-weight:700;">
    ...
    <div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(220px, 1fr)); gap:14px; margin-bottom:16px;">
      ...
      <div class="panel" style="background:var(--bg-2); border:1px solid var(--border); padding:16px 18px; border-radius:12px;">
        ...
        <div style="font-size:22px; font-weight:900; color:var(--navy); margin-top:4px;">{{ $totalAktif }} Slide</div>
      </div>
      <div class="panel" style="background:var(--bg-2); border:1px solid var(--border); padding:16px 18px; border-radius:12px;">
        <div style="font-size:11px; font-weight:800; color:var(--text-3); text-transform:uppercase;">Mode Presentasi Web</div>
        <div style="font-size:13px; font-weight:900; color:var(--text); margin-top:8px;">
          @if($totalAktif > 1)
            <span style="color:var(--navy);"><i class="bi bi-sliders"></i> Multi-Slide Slider Otomatis</span>
          @elseif($totalAktif == 1)
            <span style="color:#0ea5e9;"><i class="bi bi-check2-circle"></i> Single Hero Tetap</span>
          @else
        ...
        <table class="table" style="width:100%; border-collapse:collapse; font-size:12.5px; margin:0;">
          <thead>
            <tr style="background:var(--bg-3); border-bottom:2px solid var(--border); text-align:left;">
              <th style="padding:10px 14px; width:60px; text-align:center; color:var(--text); font-weight:800;">Urutan</th>
              <th style="padding:10px 14px; width:140px; color:var(--text); font-weight:800;">Visual Foto</th>
              <th style="padding:10px 14px; color:var(--text); font-weight:800;">Headline &amp; Narasi</th>
              <th style="padding:10px 14px; color:var(--text); font-weight:800;">Tombol Aksi (CTA)</th>
              <th style="padding:10px 14px; text-align:center; color:var(--text); font-weight:800;">Status</th>
              <th style="padding:10px 14px; text-align:center; width:140px; color:var(--text); font-weight:800;">Aksi</th>
            </tr>
          </thead>
          <tbody>
            @forelse($banners as $banner)
              <tr style="border-bottom:1px solid var(--border); vertical-align:middle;">
                {{-- Urutan --}}
                <td style="padding:12px 14px; text-align:center; font-weight:900; color:var(--text);">
                  #{{ $banner->urutan }}
                </td>
                ...
                <td style="padding:12px 14px;">
                  <div style="display:flex; align-items:center; gap:6px; margin-bottom:4px; flex-wrap:wrap;">
                    @if($banner->posisi === 'ppdb_callout')
                      <span style="font-size:10px; font-weight:800; color:#38bdf8; background:rgba(56,189,248,0.15); border:1px solid rgba(56,189,248,0.3); padding:2px 8px; border-radius:4px; text-transform:uppercase;">
                        <i class="bi bi-person-check-fill"></i> Callout PPDB
                      </span>
                    @elseif($banner->posisi === 'hero_home')
                      <span style="font-size:10px; font-weight:800; color:#ffffff; background:var(--navy); padding:2px 8px; border-radius:4px; text-transform:uppercase;">
                        <i class="bi bi-images"></i> Hero Slider
                      </span>
                    @else
                      <span style="font-size:10px; font-weight:800; color:var(--text-3); background:var(--bg-3); border:1px solid var(--border-2); padding:2px 8px; border-radius:4px; text-transform:uppercase;">
                        {{ $banner->posisi }}
                      </span>
                    @endif

                    @if($banner->badge_text)
                      <span style="font-size:10px; font-weight:800; color:var(--text-2); background:var(--bg-3); border:1px solid var(--border-2); padding:2px 8px; border-radius:4px; text-transform:uppercase;">
                        {{ $banner->badge_text }}
                      </span>
                    @endif
                ...
                <td style="padding:12px 14px;">
                  <div style="display:flex; flex-direction:column; gap:4px; font-size:11px;">
                    ...
                    @if($banner->tombol_teks_3)
                      <span style="background:rgba(16,185,129,0.12); border:1px solid rgba(16,185,129,0.3); padding:3px 8px; border-radius:4px; color:#10b981; display:inline-flex; align-items:center; gap:4px;">
                ...
                <td style="padding:12px 14px; text-align:center;">
                  <form action="{{ route('admin.banner.toggle', $banner->id) }}" method="POST" style="display:inline-block;">
                    @csrf
                    @if($banner->is_active)
                      <button type="submit" title="Klik untuk nonaktifkan" class="btn btn-sm" style="background:rgba(16,185,129,0.15); color:#10b981; border:1px solid rgba(16,185,129,0.3); font-weight:800; font-size:11px; padding:4px 10px; border-radius:20px;">
                        <i class="bi bi-check-circle-fill"></i> AKTIF
                      </button>
                    @else
                      <button type="submit" title="Klik untuk aktifkan" class="btn btn-sm" style="background:rgba(239,68,68,0.12); color:#ef4444; border:1px solid rgba(239,68,68,0.3); font-weight:800; font-size:11px; padding:4px 10px; border-radius:20px;">
                ...
                <td style="padding:12px 14px; text-align:center;">
                  <div style="display:inline-flex; gap:6px;">
                    <a href="{{ route('admin.banner.edit', $banner->id) }}" class="btn btn-sm" style="background:var(--bg-3); border:1px solid var(--border-2); color:var(--navy); padding:5px 10px; font-size:11.5px; border-radius:6px; font-weight:800; text-decoration:none;">
                      <i class="bi bi-pencil-square"></i> Edit
                    </a>
                    <form action="{{ route('admin.banner.destroy', $banner->id) }}" method="POST" onsubmit="return confirm('Hapus hero banner ini?')">
                      @csrf
                      @method('DELETE')
                      <button type="submit" class="btn btn-sm" style="background:rgba(239,68,68,0.12); border:1px solid rgba(239,68,68,0.3); color:#ef4444; padding:5px 9px; font-size:11.5px; border-radius:6px; font-weight:700;">
